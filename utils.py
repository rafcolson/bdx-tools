...
def dimensions_transformed(ob):
    bb_crns = [ob.matrix_world * Vector(corner) for corner in ob.bound_box]
    dim_x = max(bb_crns[i][0] for i in range(8)) - min(bb_crns[i][0] for i in range(8))
    dim_y = max(bb_crns[i][1] for i in range(8)) - min(bb_crns[i][1] for i in range(8))
    dim_z = max(bb_crns[i][2] for i in range(8)) - min(bb_crns[i][2] for i in range(8))
